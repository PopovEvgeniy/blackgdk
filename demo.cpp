...
int main(void)
{
 ...
 while(screen.sync()==false)
 {
  ...
  if(gamepad.check_press(IGF_GAMEPAD_X)==true) break;
  if(gamepad.check_hold(IGF_GAMEPAD_UP)==true) y-=2;
  if(gamepad.check_hold(IGF_GAMEPAD_DOWN)==true) y+=2;
  if(gamepad.check_hold(IGF_GAMEPAD_LEFT)==true) x-=2;
  if(gamepad.check_hold(IGF_GAMEPAD_RIGHT)==true) x+=2;
  if(gamepad.get_stick_x(IGF_GAMEPAD_LEFT_STICK)==1) x+=2;
  if(gamepad.get_stick_x(IGF_GAMEPAD_LEFT_STICK)==-1) x-=2;
  if(gamepad.get_stick_y(IGF_GAMEPAD_LEFT_STICK)==1) y-=2;
  if(gamepad.get_stick_y(IGF_GAMEPAD_LEFT_STICK)==-1) y+=2;
  if(gamepad.check_trigger(IGF_GAMEPAD_LEFT_TRIGGER)==true) gamepad.set_vibration(65535,65535);
  if(gamepad.check_trigger(IGF_GAMEPAD_RIGHT_TRIGGER)==true) gamepad.set_vibration(0,0);
  if(gamepad.check_press(IGF_GAMEPAD_A)==true) ship.mirror_image(0);
  if(gamepad.check_press(IGF_GAMEPAD_B)==true) ship.mirror_image(1);
